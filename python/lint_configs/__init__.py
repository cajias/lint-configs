...
def copy_config_to_project(
    ...
    Args:
        ...
        merge_with_existing: If True, will merge with existing pyproject.toml
            (manual merge required). If False (default), creates standalone file.
    ...
    if merge_with_existing:
        ...
        if target_file.exists():
            print(f"✓ pyproject.toml exists at {target_file}")
            print("⚠ Manual merge required:")
            print(f"  1. Copy sections from: {source_config}")
            print(f"  2. Paste into: {target_file}")
            print("  3. Customize known-first-party, source, and module overrides")
            return target_file
        # If doesn't exist, fall through to copy
    
    # Copy to pyproject-linters.toml to avoid overwriting existing pyproject.toml
    target_file = target_dir / "pyproject-linters.toml"
    
    # Read source and add attribution header
...
# 
# To update: pip install --upgrade agentic-guardrails && python -c "from lint_configs import copy_config_to_project; copy_config_to_project()"
#
